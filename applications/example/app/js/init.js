global.onLoad(function() {
  ...
  // QR generation
  $(document).on('click', '#menuQR', function() {
    window.location = '/qr-code';
  });

  $(document).on('click', '#submit_qr', function() {
    var param = $('#text_encode').val();
    if(param) {
      $('#qr-image').attr('src', "http://127.0.0.1/qr-code/?to_encode=" + param);
      $('#qr-image').show();
    }else{
      $('#qr-image').hide();
    }
  ...
  $(document).on('submit', '#download_qr', function(e){
    var message = $('#text_encode').val();
    if(message){
      $('#hidden_mes').val(message);
    }else{
      e.preventDefault();
    }
  });
  ...
  $(document).on('click', '#download', function() {
    var parameter = $('#text_encode').val();
    $.post('/qr-code/download.json', {'func': 'download', 'text': parameter}, function (res) {
      console.log(res);
    });
  });
